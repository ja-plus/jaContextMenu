...
class MenuItem {
    ...
    calcPosition() {
        ...
        let translateY = -2;
        if (_storeWindowSize.clientWidth - liPosition.x - parentWidth < parentWidth) {
...
class Panel {
    ...
    calcPosition(e) {
        ...
        this.width = width;
        let { x, y } = e;
        if (_storeWindowSize.clientWidth - x < width) {
            x = _storeWindowSize.clientWidth - width;
        }
        if (_storeWindowSize.clientHeight - y < height) {
            y = e.y - height;
        }
        return { x, y };
    }
