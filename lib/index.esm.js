...
function dealTextFmt(data, payload) {
    ...
    init() {
        ...
        if (item.type === 'hr' || item.type === '---') {
            ...
            this.el = h('li', {
                ...
                item.icon && h('img.menu-item-icon', { src: dealTextFmt(item.icon, this.parentMenu.payload) }),
                ...[
                    item.customItem
                        ? item.customItem
                ...
                item.tip && h('span.menu-item-tip', dealTextFmt(item.tip, this.parentMenu.payload)),
...
class Panel {
    constructor(panelOption) {
        ...
        this.width = (panelOption === null || panelOption === void 0 ? void 0 : panelOption.width) || config.defaultMenuWidth;
    ...
    calcPosition(e) {
        ...
        let x = e.x;
        let y = e.y;
        if (_storeWindowSize.clientWidth - e.x < this.width) {
            x = _storeWindowSize.clientWidth - this.width;
        }
        if (_storeWindowSize.clientHeight - e.y < this.height) {
            y = e.y - this.height;
        }
        return { x, y };
    }
