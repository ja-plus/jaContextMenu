...
class MenuItem {
    ...
    calcPosition() {
        ...
        let translateY = -2;
        if (_storeWindowSize.cW - liPosition.x - parentWidth < parentWidth) {
            translateX = -childWidth + 5;
        }
        if (_storeWindowSize.cH - liPosition.y + 2 < childMenuHeight) {
            translateY = -childMenuHeight + config.itemH + 2 + 1;
        }
...
class Panel {
    ...
    calcPosition(e) {
        ...
        this.width = width;
        let { x, y } = e;
        if (_storeWindowSize.cW - x < width && x >= width) {
...
exports.h = h;
//# sourceMappingURL=ja-contextmenu.js.map
